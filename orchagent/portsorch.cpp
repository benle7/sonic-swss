...
#define NPU_SI_SETTINGS_SYNC_STATUS_FIELD   "NPU_SI_SETTINGS_SYNC_STATUS"
            ...
            else
            {
                ...
                if (pCfg.admin_status.is_set)
                {
                    if (p.m_admin_state_up != pCfg.admin_status.value)
                    {
                         ...
                         */
                        if (pCfg.admin_status.value && serdes_attr.empty() && m_cmisModuleAsicSyncSupported)
                        {
                            vector<FieldValueTuple> siTuples;
                            bool siExist = m_portStateTable.get(p.m_alias, siTuples);
                            string npuSiStatus;
                            if (siExist)
                            {
                                for (const auto &fv : siTuples)
                                {
                                    if (fvField(fv) == NPU_SI_SETTINGS_SYNC_STATUS_FIELD)
                                    {
